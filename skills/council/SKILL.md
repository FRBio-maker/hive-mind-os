...
<!--
TEMPLATE NOTE — this skill is nearly runtime-agnostic and ships close to
verbatim. Localize only: (1) how your runtime launches isolated subagents
(any mechanism that starts a FRESH context works — the isolation is the
point), and (2) the persistence step, which should point at YOUR knowledge
base (see docs/wiki-protocol.md for the reference scheme).
-->

# Council

Convene four advisors for ambiguous decisions:
- the in-context voice (Architect — that's you)
- a Skeptic subagent
- a Pragmatist subagent
- a Critic subagent

This is for **decision-making under ambiguity**, not code review, implementation
...
| --- | --- |
| Verifying whether output is correct | a verification pass / code review |
| Breaking a feature into implementation steps | a planning workflow |
| Designing system architecture | an architecture/design session |
| Reviewing code for bugs or security | code review |
| Cross-vendor outside opinions | the `consult` skill |
...
The three subagent voices are launched as fresh subagents with **only the
question and relevant context**, not the full ongoing conversation. That is the
anti-anchoring mechanism: a voice that has read the whole thread will echo the
thread.
...
- do not dismiss another voice without explaining why
...
If the council materially changes the recommendation, persist it in your
knowledge base per your wiki protocol (a `decision` node in the active session
cluster on the reference fleet), or update the relevant issue tracker entry
directly if the decision changes active execution truth.
...
The value is not unanimity. The value is making the disagreement legible before
choosing.

## See also

- `skills/consult/` — same council PLUS mandatory cross-vendor external review
- `docs/decision-protocol.md` — when a structured decision fires at all
