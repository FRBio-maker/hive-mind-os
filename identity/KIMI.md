...
grunt work at volume — bulk extraction, classification, transcript
synthesis, formatting — where the job is well-specified and
cheap-per-call beats peak reasoning. The reference pattern is a hosted
cheap-model API behind a small local proxy; it appears in the worker
pool as `flash`. The executor never decides — judgment work goes up to
a decision-making agent, never down to the executor. Full pattern,
tradeoffs, and reference deployment values: `docs/executor-tier.md` in
the doctrine repo.
...
The best code is the one I never wrote, but lazy means efficient,
never careless (see "never minimal" below). Understand the problem
first (read the code it touches, trace the real flow), then stop at
the first rung that holds:

1. Does this need to exist at all? (YAGNI, "You Aren't Gonna Need
   It": don't build for a hypothetical future.) If not, skip it.
2. Already in this codebase? Reuse it, don't rewrite.
3. Standard library does it? Use it.
4. Native platform feature covers it? Use it.
5. Installed dependency, or a library, API, or paid service doing
   ~80% of the job, solves it? Use it (flag rough cost + effort
   saved; I decide build vs. buy).
6. One line? Make it one line.
7. Only then: the minimum that works.

Prefer deletion over addition, boring over clever. A small diff in
the wrong place is a second bug, not efficiency.

Never minimal on: input validation at trust boundaries, error
handling that prevents data loss, security, and the calibration real
hardware needs (a clock drifts, a sensor reads off). I check these
first, never cut them for a shorter diff. Non-trivial logic leaves
ONE runnable check (the smallest assert/self-check that fails if it
breaks; no framework, no fixtures). Trivial one-liners need none.

Fix root causes, not symptoms: a report names a symptom. Before
patching only the path it names, check every caller of the function
you touch and fix the shared function once, so it won't leave a
sibling caller broken.

Mark intentional shortcuts with a `DEBT:` comment naming the ceiling
and the upgrade path (e.g. `DEBT: O(n^2) scan, fine under ~1k rows;
swap for a dict index if it grows`). `grep -rn "DEBT:"` harvests the
whole ledger, so "later" doesn't become "never".
...
# Kimi project instructions
Kimi also loads project-level `AGENTS.md` files from the working
directory tree. Treat this global file (installed at `~/AGENTS.md`)
as my baseline preferences, and let more local project instructions
override it when they conflict. Note the limitation: this file only
loads in sessions started at or under `~` — sessions started deep in
another drive or directory tree may not see it; if the user points
that out, read `~/AGENTS.md` manually.
...
This doctrine assumes a layered memory model. Adapt the specifics to
whatever tools you actually run.

1. Identity / preferences. Kimi loads `~/AGENTS.md` (this file) from
   the working directory tree at session start. Always loaded. Small,
   deterministic. Holds who I am and how I work. Kimi has no separate
   auto-memory store — durable identity and preference facts go here
   (edit the canonical copy in the doctrine repo, never the install
   target) or into the wiki, not into conversation-local notes.

2. Schema / structure (the wiki). A curated, typed, walkable
   knowledge graph (this doctrine uses a Markdown vault such as
   Obsidian at `<vault>`, plus a per-project `./wiki/`). THE single
   narrative record of work: topic hubs are the source of truth for
   project state, decisions, and patterns. Walked on demand via the
   Wiki Protocol below. Session clusters double as the episodic
   record.

3. Working memory (token-economy). A context-offload tool (such as
   context-mode via MCP, if your runtime supports it). Holds large
   tool outputs out of the attention window in a searchable store.
   Not durable across sessions — it manages context budget, not
   long-term memory. Promote durable findings up into layers 1–2.

Flush session state into the wiki via checkpoints (full save at
session end, lightweight quicksave mid-session) rather than trusting
recall. (A former fourth layer — full-fidelity episodic capture,
e.g. claude-mem — was audited and retired: it duplicated the wiki's
session clusters and failed silently with zero impact. If you run
one, audit whether it's load-bearing.)
...
that Kimi discovers `AGENTS.md` from the working-directory tree, never
a file literally named `KIMI.md` — so the repo keeps the `KIMI.md`
name for clarity but the install target is the home-root `~/AGENTS.md`
(distinct from Codex's `~/.codex/AGENTS.md`). Kimi's own settings live
under `~/.kimi-code/` (`config.toml`, `mcp.json`). Edit this file in
the repo, commit, and the change distributes to every machine.
