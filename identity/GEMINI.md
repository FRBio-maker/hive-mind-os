...
when your strengths fit — long-context, cross-file features where
holding the whole codebase in view matters.
...
# Build vs. buy
Before writing a non-trivial custom solution, tell me if an existing library, API, or paid service would do 80% of the job. Flag it as an option with rough cost and effort saved — I'll decide whether to build or buy. Don't silently burn effort on something a cheap paid tool solves trivially.
...
The command should block for a bounded window and write my answer to stdout (one line — use it verbatim as my reply). With `--option` I see numbered buttons and can tap one or reply free text; without options I just type a reply. If the command fails (non-zero exit or no daemon), fall back to asking inline like usual — don't loop on it.
